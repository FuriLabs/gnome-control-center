...
static const struct {
    ...
    { 1 * 60, 20 * 60 }, { 2 * 60, 20 * 60 }, { 3 * 60, 30 * 60 }, { 5 * 60, 30 * 60 }, { 0, 0 }, /* 0-terminator */
};
...
static void enable_screen_time_recording_button_clicked_cb (GtkButton *button, gpointer user_data);
static void disable_screen_time_recording_button_clicked_cb (GtkButton *button, gpointer user_data);
...
static void movement_break_schedule_notify_selected_item_cb (GObject *object, GParamSpec *pspec, gpointer user_data);
static void break_settings_changed_duration_or_interval_cb (GSettings *settings, const char *key, gpointer user_data);
static void break_settings_writable_changed_duration_or_interval_cb (GSettings *settings, const char *key,
                                                                     gpointer user_data);

static void sounds_notify_active_cb (GObject *object, GParamSpec *pspec, gpointer user_data);
static void break_settings_changed_play_sound_cb (GSettings *settings, const char *key, gpointer user_data);
static void break_settings_writable_changed_play_sound_cb (GSettings *settings, const char *key, gpointer user_data);
static void break_settings_changed_selected_breaks_cb (GSettings *settings, const char *key, gpointer user_data);
static gboolean seconds_to_minutes (GValue *value, GVariant *variant, gpointer user_data);
static GVariant *minutes_to_seconds (const GValue *value, const GVariantType *expected_type, gpointer user_data);
...
settings_strv_binding_data_new (GSettings *settings, const char *key, const char *expected_member)
{
...
static gboolean
settings_strv_contains_str (GValue *value, GVariant *variant, gpointer user_data)
{
...
static GVariant *
settings_strv_add_or_remove_str (const GValue *value, const GVariantType *expected_type, gpointer user_data)
{
...
#ifdef HAVE_MALCONTENT
static void
get_session_limits_cb (GObject *source, GAsyncResult *res, CcWellbeingPanel *self)
{

    g_autoptr(GError) error = NULL;

    g_clear_pointer (&self->limits, mct_session_limits_unref);
    self->limits = mct_manager_get_session_limits_finish (self->policy_manager, res, &error);

    if (error && !g_error_matches (error, MCT_MANAGER_ERROR, MCT_MANAGER_ERROR_DISABLED))
...
static void
session_limits_changed_cb (CcWellbeingPanel *self, uid_t uid)
{
    if (uid != getuid ())
        return;

    mct_manager_get_session_limits_async (self->policy_manager, getuid (), MCT_MANAGER_GET_VALUE_FLAGS_NONE,
                                          cc_panel_get_cancellable (CC_PANEL (self)),
                                          (GAsyncReadyCallback) get_session_limits_cb, self);
}

static void
system_bus_ready (GObject *source, GAsyncResult *res, CcWellbeingPanel *self)
{
    ...
    if (!bus) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
            g_warning ("Error getting system bus while setting up session limits: %s", error->message);
        }
        return;
    }
    ...
    g_signal_connect_object (self->policy_manager, "session-limits-changed", G_CALLBACK (session_limits_changed_cb),
...
{
    g_autoptr(GError) error = NULL;
    const gchar *argv[] = { "malcontent-control", "--user", g_get_user_name (), NULL };
...
{
    g_settings_bind (self->screen_time_limits_settings, "daily-limit-enabled", self->screen_time_limit_row, "active",
                     G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_NO_SENSITIVITY);
    g_settings_bind_with_mapping (self->screen_time_limits_settings, "daily-limit-seconds", self->daily_time_limit_row,
                                  "duration", G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_NO_SENSITIVITY,
                                  seconds_to_minutes, minutes_to_seconds, NULL, NULL);
    g_settings_bind (self->screen_time_limits_settings, "grayscale", self->grayscale_row, "active",
                     G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_NO_SENSITIVITY);
...
cc_wellbeing_panel_init (CcWellbeingPanel *self)
{
    ...
    self->limits = NULL;
    g_bus_get (G_BUS_TYPE_SYSTEM, cc_panel_get_cancellable (CC_PANEL (self)), (GAsyncReadyCallback) system_bus_ready,
               self);
    ...
    g_signal_connect_object (self->parental_controls_banner, "button-clicked", G_CALLBACK (spawn_malcontent_control),
                             self, G_CONNECT_SWAPPED);
#endif
    ...
    g_settings_bind_with_mapping (
        self->break_reminders_settings, "selected-breaks", self->eyesight_breaks_row, "active", G_SETTINGS_BIND_DEFAULT,
        settings_strv_contains_str, settings_strv_add_or_remove_str,
    ...
    g_settings_bind_with_mapping (
        self->break_reminders_settings, "selected-breaks", self->movement_breaks_row, "active", G_SETTINGS_BIND_DEFAULT,
        settings_strv_contains_str, settings_strv_add_or_remove_str,
    ...
    self->sounds_notify_active_id =
        g_signal_connect (self->sounds_row, "notify::active", G_CALLBACK (sounds_notify_active_cb), self);
    self->eyesight_break_settings_changed_play_sound_id = g_signal_connect (
        self->eyesight_break_settings, "changed::play-sound", G_CALLBACK (break_settings_changed_play_sound_cb), self);
    self->movement_break_settings_changed_play_sound_id = g_signal_connect (
        self->movement_break_settings, "changed::play-sound", G_CALLBACK (break_settings_changed_play_sound_cb), self);
    ...
    history_file =
        g_file_new_build_filename (g_get_user_data_dir (), "gnome-shell", "session-active-history.json", NULL);
    cc_screen_time_statistics_row_set_history_file (self->screen_time_statistics_row, history_file);
}

static void
enable_screen_time_recording_button_clicked_cb (GtkButton *button, gpointer user_data)
{
...
static void
disable_screen_time_recording_button_clicked_cb (GtkButton *button, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
...
{
    gboolean history_enabled = g_settings_get_boolean (self->screen_time_limits_settings, "history-enabled");
    gboolean daily_limit_enabled_writable =
        g_settings_is_writable (self->screen_time_limits_settings, "daily-limit-enabled");
    gboolean daily_limit_enabled = g_settings_get_boolean (self->screen_time_limits_settings, "daily-limit-enabled");
    gboolean daily_limit_seconds_writable =
        g_settings_is_writable (self->screen_time_limits_settings, "daily-limit-seconds");
    ...
    gtk_widget_set_sensitive (GTK_WIDGET (self->screen_time_limit_row),
                              !session_limits_enabled && history_enabled && daily_limit_enabled_writable);
    gtk_widget_set_sensitive (GTK_WIDGET (self->daily_time_limit_row), !session_limits_enabled && history_enabled
                                                                           && daily_limit_enabled
                                                                           && daily_limit_seconds_writable);
    gtk_widget_set_sensitive (GTK_WIDGET (self->grayscale_row),
                              !session_limits_enabled && history_enabled && daily_limit_enabled && grayscale_writable);
    gtk_widget_set_sensitive (GTK_WIDGET (self->screen_time_statistics_info_button), !session_limits_enabled);
}
...
static void
movement_break_schedule_notify_selected_item_cb (GObject *object, GParamSpec *pspec, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
    CcBreakSchedule *selected_item =
        CC_BREAK_SCHEDULE (adw_combo_row_get_selected_item (self->movement_break_schedule_row));

    g_settings_set_uint (self->movement_break_settings, "duration-seconds",
                         cc_break_schedule_get_duration_secs (selected_item));
    g_settings_set_uint (self->movement_break_settings, "interval-seconds",
                         cc_break_schedule_get_interval_secs (selected_item));
    g_settings_apply (self->movement_break_settings);
}

static gboolean
break_schedule_equal_cb (gconstpointer a, gconstpointer b)
{
    return cc_break_schedule_compare ((CcBreakSchedule *) a, (CcBreakSchedule *) b) == 0;
}

static gint
break_schedule_compare_cb (gconstpointer a, gconstpointer b, gpointer user_data)
{
    return cc_break_schedule_compare ((CcBreakSchedule *) a, (CcBreakSchedule *) b);
}

static void
break_settings_changed_duration_or_interval_cb (GSettings *settings, const char *key, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
    ...
    if (!g_list_store_find_with_equal_func (self->movement_break_schedule_list, settings_schedule,
                                            break_schedule_equal_cb, &selected_index)) {
        gboolean again;
        ...
        again = g_list_store_find_with_equal_func (self->movement_break_schedule_list, settings_schedule,
                                                   break_schedule_equal_cb, &selected_index);
        g_assert (again);
    }
...
static void
break_settings_writable_changed_duration_or_interval_cb (GSettings *settings, const char *key, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
...
static void
sounds_notify_active_cb (GObject *object, GParamSpec *pspec, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
...
static void
break_settings_changed_play_sound_cb (GSettings *settings, const char *key, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
...
static void
break_settings_writable_changed_play_sound_cb (GSettings *settings, const char *key, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
...
static void
break_settings_changed_selected_breaks_cb (GSettings *settings, const char *key, gpointer user_data)
{
    CcWellbeingPanel *self = CC_WELLBEING_PANEL (user_data);
    update_sounds_row_sensitivity (self);
    update_movement_break_schedule_row_sensitivity (self);
...
static gboolean
seconds_to_minutes (GValue *value, GVariant *variant, gpointer user_data)
{
...
static GVariant *
minutes_to_seconds (const GValue *value, const GVariantType *expected_type, gpointer user_data)
{
...
cc_wellbeing_panel_dispose (GObject *object)
{
    ...
    g_clear_signal_handler (&self->screen_time_limits_settings_writable_changed_daily_limit_seconds_id,
                            self->screen_time_limits_settings);
    g_clear_signal_handler (&self->screen_time_limits_settings_writable_changed_grayscale_id,
                            self->screen_time_limits_settings);
    g_clear_signal_handler (&self->screen_time_limits_settings_writable_changed_daily_limit_enabled_id,
                            self->screen_time_limits_settings);
    g_clear_signal_handler (&self->screen_time_limits_settings_changed_daily_limit_enabled_id,
                            self->screen_time_limits_settings);
    ...
    g_clear_signal_handler (&self->eyesight_break_settings_writable_changed_play_sound_id,
                            self->eyesight_break_settings);
    g_clear_signal_handler (&self->movement_break_settings_writable_changed_play_sound_id,
                            self->movement_break_settings);
...
cc_wellbeing_panel_class_init (CcWellbeingPanelClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/wellbeing/cc-wellbeing-panel.ui");
