...
airplane_mode_changed_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!g_dbus_proxy_call_finish (G_DBUS_PROXY (source_object), res, &error)) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
            ...
            g_warning ("Failed to change Bluetooth killswitch state to %s: %s", state ? "on" : "off", error->message);
        }
    } else {
        CcBluetoothPanel *self = CC_BLUETOOTH_PANEL (user_data);
        gboolean state = gtk_switch_get_active (self->enable_switch);
...
enable_switch_state_set_cb (CcBluetoothPanel *self, gboolean state)
{
    ...
    g_dbus_proxy_call (self->properties, "Set",
                       ...
                       G_DBUS_CALL_FLAGS_NONE, -1, cc_panel_get_cancellable (CC_PANEL (self)), airplane_mode_changed_cb,
                       self);
...
cc_bluetooth_panel_init (CcBluetoothPanel *self)
{
    ...
    g_signal_connect_object (self->rfkill, "g-properties-changed", G_CALLBACK (airplane_mode_changed), self,
                             G_CONNECT_SWAPPED);
}
