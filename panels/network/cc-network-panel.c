...
static void
cc_network_panel_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    switch (property_id) {
    default:
...
static gboolean
verify_argv (CcNetworkPanel *self, const char **args)
{
...
static void
cc_network_panel_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
...
static gboolean
handle_argv_for_connection (CcNetworkPanel *self, NMConnection *connection)
{
    if (self->arg_operation == OPERATION_NULL)
        return TRUE;
...
panel_add_device (CcNetworkPanel *self, NMDevice *device)
{
    ...
    g_debug ("device %s type %i path %s", nm_device_get_udi (device), type, nm_object_get_path (NM_OBJECT (device)));
    ...
    case NM_DEVICE_TYPE_MODEM:
        if (g_str_has_prefix (nm_device_get_udi (device), "/org/freedesktop/ModemManager1/Modem/")) {
            ...
            if (modem_object == NULL) {
                g_warning ("Cannot grab information for modem at %s: Not found", nm_device_get_udi (device));
                return;
            }
...
active_connections_changed (CcNetworkPanel *self)
{
    ...
    for (i = 0; connections && (i < connections->len); i++) {
        ...
        if (g_object_get_data (G_OBJECT (connection), "has-state-changed-handler") == NULL) {
            g_signal_connect_object (connection, "notify::state", G_CALLBACK (connection_state_changed), self,
                                     G_CONNECT_SWAPPED);
...
device_removed_cb (CcNetworkPanel *self, NMDevice *device)
{
    ...
    g_signal_handlers_disconnect_by_func (device, G_CALLBACK (device_managed_cb), self);
}
...
static gint
sort_vpns_func (GtkListBoxRow *a, GtkListBoxRow *b, gpointer user_data)
{
...
add_connection (CcNetworkPanel *self, NMConnection *connection)
{
    ...
    g_debug ("add %s/%s remote connection: %s", type, g_type_name_from_instance ((GTypeInstance *) connection),
...
cc_network_panel_init (CcNetworkPanel *self)
{
    ...
    g_signal_connect_object (self->client, "notify::nm-running", G_CALLBACK (manager_running), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->client, "notify::active-connections", G_CALLBACK (active_connections_changed), self,
                             G_CONNECT_SWAPPED);
    ...
    g_signal_connect_object (self->client, NM_CLIENT_CONNECTION_ADDED, G_CALLBACK (add_connection), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->client, NM_CLIENT_CONNECTION_REMOVED, G_CALLBACK (client_connection_removed_cb),
