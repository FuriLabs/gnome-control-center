...
static void update_apply_button (CcDisplayPanel *self);
static void apply_current_configuration (CcDisplayPanel *self);
static void cancel_current_configuration (CcDisplayPanel *panel);
static void reset_current_config (CcDisplayPanel *self);
static void rebuild_ui (CcDisplayPanel *self);
static void set_current_output (CcDisplayPanel *self, CcDisplayMonitor *output, gboolean force);
static void on_screen_changed (CcDisplayPanel *self);
...
config_ensure_of_type (CcDisplayPanel *self, CcDisplayConfigType type)
{
    ...
    case CC_DISPLAY_CONFIG_JOIN:
        ...
        for (l = outputs; l; l = l->next) {
            ...
            if (!cc_display_config_is_scaled_mode_valid (self->current_config, mode, scale)) {
                if (current_primary
                    && cc_display_config_is_scaled_mode_valid (self->current_config, mode, old_primary_scale))
    ...
    case CC_DISPLAY_CONFIG_CLONE: {
        ...
        if (current_primary
            && cc_display_config_is_scaled_mode_valid (self->current_config, clone_mode, old_primary_scale))
            scale = old_primary_scale;
...
monitor_labeler_hide (CcDisplayPanel *self)
{
    ...
    g_dbus_proxy_call (self->shell_proxy, "HideMonitorLabels", NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
}
...
monitor_labeler_show (CcDisplayPanel *self)
{
    ...
    for (l = outputs; l != NULL; l = l->next) {
        ...
        g_variant_builder_add (&builder, "{sv}", cc_display_monitor_get_connector_name (output),
    ...
    g_dbus_proxy_call (self->shell_proxy, "ShowMonitorLabels", g_variant_builder_end (&builder), G_DBUS_CALL_FLAGS_NONE,
                       -1, NULL, NULL, NULL);
}
...
static void
cc_display_panel_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcDisplayPanel *self = CC_DISPLAY_PANEL (object);
...
static void
on_monitor_settings_updated_cb (CcDisplayPanel *self, CcDisplayMonitor *monitor, CcDisplaySettings *settings)
{
    if (monitor)
        cc_display_config_snap_outputs (self->current_config);
...
static gboolean
on_toplevel_escape_pressed_cb (GtkWidget *widget, GVariant *args, CcDisplayPanel *self)
{
...
cc_display_panel_constructed (GObject *object)
{
    ...
    g_signal_connect_object (toplevel, "notify::active-panel", G_CALLBACK (active_panel_changed), object,
                             G_CONNECT_SWAPPED);
...
cc_display_panel_class_init (CcDisplayPanelClass *klass)
{
    ...
    properties[PROP_SHOWING_APPLY_TITLEBAR] =
        g_param_spec_boolean ("showing-apply-titlebar", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
...
static void
set_current_output (CcDisplayPanel *self, CcDisplayMonitor *output, gboolean force)
{
    gboolean changed;
...
static void
on_monitor_row_activated_cb (CcDisplayPanel *self, GtkListBoxRow *row)
{
...
static void
add_display_row (CcDisplayPanel *self, CcDisplayMonitor *monitor)
{
    ...
    adw_preferences_row_set_title (ADW_PREFERENCES_ROW (row), cc_display_monitor_get_ui_name (monitor));
...
move_display_settings_to_main_page (CcDisplayPanel *self)
{
    ...
    adw_preferences_group_add (self->single_display_settings_group, GTK_WIDGET (self->settings));
...
move_display_settings_to_separate_page (CcDisplayPanel *self)
{
    ...
    g_object_ref (self->settings);
    adw_preferences_group_remove (self->single_display_settings_group, GTK_WIDGET (self->settings));
...
rebuild_ui (CcDisplayPanel *self)
{
    ...
    while (self->monitor_rows) {
        adw_preferences_group_remove (ADW_PREFERENCES_GROUP (self->display_settings_group), self->monitor_rows->data);
    ...
    cc_display_settings_set_multimonitor (self->settings, n_usable_outputs > 1 && type != CC_DISPLAY_CONFIG_CLONE);
...
static void
update_panel_orientation_managed (CcDisplayPanel *self, gboolean managed)
{
...
reset_current_config (CcDisplayPanel *self)
{
    ...
    g_signal_connect_object (current, "panel-orientation-managed", G_CALLBACK (update_panel_orientation_managed), self,
                             G_CONNECT_SWAPPED);
    update_panel_orientation_managed (self, cc_display_config_get_panel_orientation_managed (current));
...
show_apply_titlebar (CcDisplayPanel *self, gboolean is_applicable)
{
    ...
    if (is_applicable) {
        adw_window_title_set_title (self->apply_titlebar_title_widget, _("Apply Changes?"));
        adw_window_title_set_subtitle (self->apply_titlebar_title_widget, "");
    } else {
        adw_window_title_set_title (self->apply_titlebar_title_widget, _("Changes Cannot be Applied"));
        adw_window_title_set_subtitle (self->apply_titlebar_title_widget,
...
update_apply_button (CcDisplayPanel *self)
{
    ...
    config_equal = cc_display_config_equal (self->current_config, applied_config);
...
mapped_cb (CcDisplayPanel *self)
{
    ...
    if (toplevel && !self->focus_id)
        self->focus_id = g_signal_connect_object (
            toplevel, "notify::is-active", G_CALLBACK (dialog_toplevel_is_active_changed), self, G_CONNECT_SWAPPED);
}
...
static void
shell_proxy_ready (GObject *source, GAsyncResult *res, CcDisplayPanel *self)
{
    GDBusProxy *proxy;
    g_autoptr(GError) error = NULL;
...
static void
session_bus_ready (GObject *source, GAsyncResult *res, CcDisplayPanel *self)
{
    GDBusConnection *bus;
    g_autoptr(GError) error = NULL;
    ...
    g_signal_connect_object (self->manager, "changed", G_CALLBACK (on_screen_changed), self, G_CONNECT_SWAPPED);
}
...
cc_display_panel_init (CcDisplayPanel *self)
{
    ...
    g_signal_connect_object (self->arrangement, "updated", G_CALLBACK (update_apply_button), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->arrangement, "notify::selected-output",
                             G_CALLBACK (on_arrangement_selected_ouptut_changed_cb), self, G_CONNECT_SWAPPED);

    self->settings = cc_display_settings_new (self);
    adw_bin_set_child (self->display_settings_bin, GTK_WIDGET (self->settings));
    g_signal_connect_object (self->settings, "updated", G_CALLBACK (on_monitor_settings_updated_cb), self,
                             G_CONNECT_SWAPPED);
    ...
    expression = gtk_cclosure_expression_new (G_TYPE_STRING, NULL, 0, NULL,
                                              G_CALLBACK (cc_display_monitor_dup_ui_number_name), self, NULL);
    adw_combo_row_set_expression (self->primary_display_row, expression);
    adw_combo_row_set_model (self->primary_display_row, G_LIST_MODEL (self->primary_display_list));
    ...
    cc_object_storage_create_dbus_proxy (
        ...
        "org.gnome.Shell", "/org/gnome/Shell", "org.gnome.Shell", cc_panel_get_cancellable (CC_PANEL (self)),
        (GAsyncReadyCallback) shell_proxy_ready, self);

    g_bus_get (G_BUS_TYPE_SESSION, cc_panel_get_cancellable (CC_PANEL (self)), (GAsyncReadyCallback) session_bus_ready,
               self);

    gtk_shortcut_set_action (self->escape_shortcut,
                             gtk_callback_action_new ((GtkShortcutFunc) on_toplevel_escape_pressed_cb, self, NULL));

    self->display_settings = g_settings_new (DISPLAY_SCHEMA);
    g_signal_connect_object (self->display_settings, "changed::night-light-enabled",
                             G_CALLBACK (on_night_light_enabled_changed_cb), self, G_CONNECT_SWAPPED);
