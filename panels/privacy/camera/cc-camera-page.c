...
G_DEFINE_FINAL_TYPE (CcCameraPage, cc_camera_page, ADW_TYPE_NAVIGATION_PAGE)
...
static void
camera_app_state_data_free (CameraAppStateData *data, GClosure *closure)
{
    g_free (data->app_id);
...
static gboolean
on_camera_app_state_set (GtkSwitch *widget, gboolean state, gpointer user_data)
{
    ...
    active_camera = !g_settings_get_boolean (self->privacy_settings, "disable-camera");
    ...
    params = g_variant_new ("(sbsa{sas}v)", APP_PERMISSIONS_TABLE, TRUE, APP_PERMISSIONS_ID, &builder,
                            self->camera_apps_data);

    g_dbus_proxy_call (self->perm_store, "Set", params, G_DBUS_CALL_FLAGS_NONE, -1, self->cancellable,
                       on_perm_store_set_done, data);

    return TRUE;
}

static gboolean
update_app_switch_state (GValue *value, GVariant *variant, gpointer data)
{
...
static void
add_camera_app (CcCameraPage *self, const gchar *app_id, gboolean enabled)
{
    ...
    g_settings_bind_with_mapping (self->privacy_settings, "disable-camera", w, "state", G_SETTINGS_BIND_GET,
                                  update_app_switch_state, NULL, g_object_ref (w), g_object_unref);

    g_hash_table_insert (self->camera_app_switches, g_strdup (app_id), g_object_ref (w));

    g_hash_table_insert (self->camera_app_rows, g_strdup (app_id), g_object_ref (row));
    ...
    data->widget = w;
    data->changing_state = FALSE;
    g_signal_connect_data (G_OBJECT (w), "state-set", G_CALLBACK (on_camera_app_state_set), data,
                           (GClosureNotify) camera_app_state_data_free, 0);
}

/* Steals permissions and permissions_data references */
static void
update_perm_store (CcCameraPage *self, GVariant *permissions, GVariant *permissions_data)
{
    GVariantIter iter;
    const gchar *key;
    gchar **value;
...
static void
on_perm_store_signal (GDBusProxy *proxy, gchar *sender_name, gchar *signal_name, GVariant *parameters,
                      gpointer user_data)
{
    ...
    if (g_strcmp0 (g_variant_get_string (table, NULL), "devices") != 0
        || g_strcmp0 (g_variant_get_string (id, NULL), "camera") != 0)
        return;
...
static void
on_perm_store_lookup_done (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    g_signal_connect_object (source_object, "g-signal", G_CALLBACK (on_perm_store_signal), self, 0);
}
...
on_perm_store_ready (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    params = g_variant_new ("(ss)", APP_PERMISSIONS_TABLE, APP_PERMISSIONS_ID);
    g_dbus_proxy_call (self->perm_store, "Lookup", params, G_DBUS_CALL_FLAGS_NONE, -1, self->cancellable,
                       on_perm_store_lookup_done, self);
}
...
cc_camera_page_class_init (CcCameraPageClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/privacy/camera/cc-camera-page.ui");
...
cc_camera_page_init (CcCameraPage *self)
{
    ...
    g_settings_bind (self->privacy_settings, "disable-camera", self->camera_row, "active",
                     G_SETTINGS_BIND_INVERT_BOOLEAN);

    self->camera_app_switches = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_object_unref);
    self->camera_app_rows = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_object_unref);

    g_dbus_proxy_new_for_bus (
        G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.freedesktop.impl.portal.PermissionStore",
        "/org/freedesktop/impl/portal/PermissionStore", "org.freedesktop.impl.portal.PermissionStore",
        self->cancellable, on_perm_store_ready, self);
