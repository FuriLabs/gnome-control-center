...
cc_wwan_device_3gpp_changed_cb (CcWwanDevice *self)
{
    ...
    if (self->modem_3gpp) {
        handler_id = g_signal_connect_object (self->modem_3gpp, "notify::registration-state",
                                              G_CALLBACK (cc_wwan_device_state_changed_cb), self, G_CONNECT_SWAPPED);
        self->modem_3gpp_id = handler_id;

        handler_id = g_signal_connect_object (self->modem_3gpp, "notify::enabled-facility-locks",
                                              G_CALLBACK (cc_wwan_device_locks_changed_cb), self, G_CONNECT_SWAPPED);
...
#if defined(HAVE_NETWORK_MANAGER) && defined(BUILD_NETWORK)
static void
cc_wwan_device_nm_changed_cb (CcWwanDevice *self, GParamSpec *pspec, NMClient *client)
{
...
static void
cc_wwan_device_nm_device_added_cb (CcWwanDevice *self, NMDevice *nm_device)
{
    ...
    self->wwan_data = cc_wwan_data_new (self->mm_object, NM_CLIENT (self->nm_client));

    if (self->wwan_data) {
        g_signal_connect_object (self->wwan_data, "notify::enabled", G_CALLBACK (wwan_device_emit_data_changed), self,
                                 G_CONNECT_SWAPPED);
        wwan_device_emit_data_changed (self);
    }
}
...
static void
cc_wwan_device_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
cc_wwan_device_class_init (CcWwanDeviceClass *klass)
{
    ...
    properties[PROP_OPERATOR_NAME] =
        g_param_spec_string ("operator-name", NULL, NULL, NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_ENABLED_LOCKS] =
        g_param_spec_int ("enabled-locks", NULL, NULL, MM_MODEM_3GPP_FACILITY_NONE, MM_MODEM_3GPP_FACILITY_CORP_PERS,
                          MM_MODEM_3GPP_FACILITY_NONE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_ERROR] =
        g_param_spec_boolean ("error", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_HAS_DATA] =
        g_param_spec_boolean ("has-data", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_NETWORK_MODE] =
        g_param_spec_string ("network-mode", NULL, NULL, NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_REGISTRATION_STATE] = g_param_spec_int (
        "registration-state", NULL, NULL, CC_WWAN_REGISTRATION_STATE_UNKNOWN, CC_WWAN_REGISTRATION_STATE_DENIED,
        CC_WWAN_REGISTRATION_STATE_UNKNOWN, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_UNLOCK_REQUIRED] =
        g_param_spec_int ("unlock-required", NULL, NULL, MM_MODEM_LOCK_UNKNOWN, MM_MODEM_LOCK_PH_NETSUB_PUK,
                          MM_MODEM_LOCK_UNKNOWN, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_SIGNAL] =
        g_param_spec_int ("signal", NULL, NULL, 0, 100, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
 ...
 */
CcWwanDevice *
cc_wwan_device_new (MMObject *mm_object, GObject *nm_client)
{
    CcWwanDevice *self;
...
#if defined(HAVE_NETWORK_MANAGER) && defined(BUILD_NETWORK)
        self->wwan_data = cc_wwan_data_new (mm_object, NM_CLIENT (self->nm_client));
#endif
    }

    g_signal_connect_object (self->mm_object, "notify::unlock-required", G_CALLBACK (cc_wwan_device_unlock_required_cb),
...
#if defined(HAVE_NETWORK_MANAGER) && defined(BUILD_NETWORK)
    g_signal_connect_object (self->nm_client, "notify::nm-running", G_CALLBACK (cc_wwan_device_nm_changed_cb), self,
                             G_CONNECT_SWAPPED);

    g_signal_connect_object (self->nm_client, "device-added", G_CALLBACK (cc_wwan_device_nm_device_added_cb), self,
                             G_CONNECT_SWAPPED);
#endif

    g_signal_connect_object (self->mm_object, "notify::modem3gpp", G_CALLBACK (cc_wwan_device_3gpp_changed_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->modem, "notify::signal-quality",
                             G_CALLBACK (cc_wwan_device_signal_quality_changed_cb), self, G_CONNECT_SWAPPED);

    cc_wwan_device_3gpp_changed_cb (self);
    g_signal_connect_object (self->modem, "notify::current-modes", G_CALLBACK (cc_wwan_device_mode_changed_cb), self,
                             G_CONNECT_SWAPPED);
...
guint
cc_wwan_device_get_unlock_retries (CcWwanDevice *self, MMModemLock lock)
{
...
static void
cc_wwan_device_pin_sent_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
...
void
cc_wwan_device_send_pin (CcWwanDevice *self, const gchar *pin, GCancellable *cancellable, GAsyncReadyCallback callback,
                         gpointer user_data)
{
    ...
    mm_sim_send_pin (self->sim, pin, cancellable, cc_wwan_device_pin_sent_cb, g_steal_pointer (&task));
}

gboolean
cc_wwan_device_send_pin_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
...
static void
cc_wwan_device_puk_sent_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
    MMSim *sim = (MMSim *) object;
    g_autoptr(GTask) task = user_data;
...
void
cc_wwan_device_send_puk (CcWwanDevice *self, const gchar *puk, const gchar *pin, GCancellable *cancellable,
                         GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    mm_sim_send_puk (self->sim, puk, pin, cancellable, cc_wwan_device_puk_sent_cb, g_steal_pointer (&task));
}

gboolean
cc_wwan_device_send_puk_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
static void
cc_wwan_device_enable_pin_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
    MMSim *sim = (MMSim *) object;
    g_autoptr(GTask) task = user_data;
...
void
cc_wwan_device_enable_pin (CcWwanDevice *self, const gchar *pin, GCancellable *cancellable,
                           GAsyncReadyCallback callback, gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
    ...
    mm_sim_enable_pin (self->sim, pin, cancellable, cc_wwan_device_enable_pin_cb, g_steal_pointer (&task));
}

gboolean
cc_wwan_device_enable_pin_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
static void
cc_wwan_device_disable_pin_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
    MMSim *sim = (MMSim *) object;
    g_autoptr(GTask) task = user_data;
...
void
cc_wwan_device_disable_pin (CcWwanDevice *self, const gchar *pin, GCancellable *cancellable,
                            GAsyncReadyCallback callback, gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
    ...
    mm_sim_disable_pin (self->sim, pin, cancellable, cc_wwan_device_disable_pin_cb, g_steal_pointer (&task));
}

gboolean
cc_wwan_device_disable_pin_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
static void
cc_wwan_device_change_pin_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
    MMSim *sim = (MMSim *) object;
    g_autoptr(GTask) task = user_data;
...
void
cc_wwan_device_change_pin (CcWwanDevice *self, const gchar *old_pin, const gchar *new_pin, GCancellable *cancellable,
                           GAsyncReadyCallback callback, gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
    ...
    mm_sim_change_pin (self->sim, old_pin, new_pin, cancellable, cc_wwan_device_change_pin_cb, g_steal_pointer (&task));
}

gboolean
cc_wwan_device_change_pin_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
static void
cc_wwan_device_network_mode_set_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
 ...
 */
void
cc_wwan_device_set_current_mode (CcWwanDevice *self, MMModemMode allowed, MMModemMode preferred,
                                 GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    task = g_task_new (self, cancellable, callback, user_data);
    permission = polkit_permission_new_sync ("org.freedesktop.ModemManager1.Device.Control", NULL, cancellable, &error);
    if (permission)
    ...
    if (error) {
        ...
        error = g_error_new (G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED, "Access Denied");
        g_clear_error (&self->error);
        self->error = g_error_copy (error);
        g_object_notify_by_pspec (G_OBJECT (self), properties[PROP_ERROR]);
        ...
        mm_modem_set_current_modes (self->modem, allowed, preferred, cancellable, cc_wwan_device_network_mode_set_cb,
 ...
 */
gboolean
cc_wwan_device_set_current_mode_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
gboolean
cc_wwan_device_get_current_mode (CcWwanDevice *self, MMModemMode *allowed, MMModemMode *preferred)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
...
gboolean
cc_wwan_device_get_supported_modes (CcWwanDevice *self, MMModemMode *allowed, MMModemMode *preferred)
{
...
gchar *
cc_wwan_device_get_string_from_mode (CcWwanDevice *self, MMModemMode allowed, MMModemMode preferred)
{
    GString *str;
    ...
    if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_3G && allowed & MM_MODEM_MODE_4G
    ...
    } else if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_3G && allowed & MM_MODEM_MODE_4G) {
    ...
    } else if (allowed & MM_MODEM_MODE_3G && allowed & MM_MODEM_MODE_4G && allowed & MM_MODEM_MODE_5G) {
        if (preferred & MM_MODEM_MODE_5G)
    ...
    } else if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_4G && allowed & MM_MODEM_MODE_5G) {
        if (preferred & MM_MODEM_MODE_5G)
    ...
    } else if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_3G && allowed & MM_MODEM_MODE_5G) {
        if (preferred & MM_MODEM_MODE_5G)
    ...
    } else if (allowed & MM_MODEM_MODE_3G && allowed & MM_MODEM_MODE_4G) {
        if (preferred & MM_MODEM_MODE_4G)
    ...
    } else if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_4G) {
        if (preferred & MM_MODEM_MODE_4G)
    ...
    } else if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_3G) {
    ...
    } else if (allowed & MM_MODEM_MODE_2G && allowed & MM_MODEM_MODE_5G) {
        if (preferred & MM_MODEM_MODE_5G)
    ...
    } else if (allowed & MM_MODEM_MODE_3G && allowed & MM_MODEM_MODE_5G) {
        if (preferred & MM_MODEM_MODE_5G)
    ...
    } else if (allowed & MM_MODEM_MODE_4G && allowed & MM_MODEM_MODE_5G) {
        if (preferred & MM_MODEM_MODE_5G)
...
static void
cc_wwan_device_scan_complete_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
...
void
cc_wwan_device_scan_networks (CcWwanDevice *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                              gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
    ...
    mm_modem_3gpp_scan (self->modem_3gpp, cancellable, cc_wwan_device_scan_complete_cb, g_steal_pointer (&task));
}

GList *
cc_wwan_device_scan_networks_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
static void
cc_wwan_device_register_network_complete_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    CcWwanDevice *self;
    MMModem3gpp *modem_3gpp = (MMModem3gpp *) object;
    g_autoptr(GTask) task = user_data;
...
void
cc_wwan_device_register_network (CcWwanDevice *self, const gchar *network_id, GCancellable *cancellable,
                                 GAsyncReadyCallback callback, gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
    ...
    mm_modem_3gpp_register (self->modem_3gpp, network_id, cancellable, cc_wwan_device_register_network_complete_cb,
                            g_steal_pointer (&task));
}

gboolean
cc_wwan_device_register_network_finish (CcWwanDevice *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), FALSE);
    g_return_val_if_fail (G_IS_TASK (result), FALSE);
...
gboolean
cc_wwan_device_is_nm_device (CcWwanDevice *self, GObject *nm_device)
{
#if defined(HAVE_NETWORK_MANAGER) && defined(BUILD_NETWORK)
    g_return_val_if_fail (NM_IS_DEVICE (nm_device), FALSE);

    return g_str_equal (mm_modem_get_primary_port (self->modem), nm_device_get_iface (NM_DEVICE (nm_device)));
#else
    return FALSE;
#endif
}
...
gboolean
cc_wwan_device_pin_valid (const gchar *password, MMModemLock lock)
{
    size_t len;

    g_return_val_if_fail (lock == MM_MODEM_LOCK_SIM_PIN || lock == MM_MODEM_LOCK_SIM_PIN2
                              || lock == MM_MODEM_LOCK_SIM_PUK || lock == MM_MODEM_LOCK_SIM_PUK2,
                          FALSE);
    if (!password)
        return FALSE;
     ...
     */
    if (lock == MM_MODEM_LOCK_SIM_PUK || lock == MM_MODEM_LOCK_SIM_PUK2)
        if (len != 8)
            return FALSE;
