...
G_DEFINE_FINAL_TYPE (CcDisplayConfigManager, cc_display_config_manager, G_TYPE_OBJECT)
...
static void
got_current_state (GObject *object, GAsyncResult *result, gpointer data)
{
    CcDisplayConfigManager *self;
    GVariant *variant;
    g_autoptr(GError) error = NULL;

    variant = g_dbus_connection_call_finish (G_DBUS_CONNECTION (object), result, &error);
    if (!variant) {
...
{
    g_dbus_connection_call (self->connection, "org.gnome.Mutter.DisplayConfig", "/org/gnome/Mutter/DisplayConfig",
                            "org.gnome.Mutter.DisplayConfig", "GetCurrentState", NULL, NULL,
                            G_DBUS_CALL_FLAGS_NO_AUTO_START, -1, self->cancellable, got_current_state, self);
}

static void
monitors_changed (GDBusConnection *connection, const gchar *sender_name, const gchar *object_path,
...
static void
bus_gotten (GObject *object, GAsyncResult *result, gpointer data)
{
    CcDisplayConfigManager *self;
    GDBusConnection *connection;
    g_autoptr(GError) error = NULL;
    ...
    self->monitors_changed_id = g_dbus_connection_signal_subscribe (
        self->connection, "org.gnome.Mutter.DisplayConfig", "org.gnome.Mutter.DisplayConfig", "MonitorsChanged",
        "/org/gnome/Mutter/DisplayConfig", NULL, G_DBUS_SIGNAL_FLAGS_NONE, monitors_changed, self, NULL);

    proxy = g_dbus_proxy_new_sync (self->connection, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.gnome.Mutter.DisplayConfig",
                                   "/org/gnome/Mutter/DisplayConfig", "org.gnome.Mutter.DisplayConfig", NULL, &error);
    if (!proxy) {
        g_warning ("Failed to create D-Bus proxy to \"org.gnome.Mutter.DisplayConfig\": %s", error->message);
        return;
    }
...
cc_display_config_manager_finalize (GObject *object)
{
    ...
    if (self->monitors_changed_id && self->connection)
        g_dbus_connection_signal_unsubscribe (self->connection, self->monitors_changed_id);
...
cc_display_config_manager_class_init (CcDisplayConfigManagerClass *klass)
{
    ...
    config_manager_signals[CONFIG_MANAGER_CHANGED] =
        g_signal_new ("changed", CC_TYPE_DISPLAY_CONFIG_MANAGER, G_SIGNAL_RUN_LAST, 0, NULL, NULL,
...
cc_display_config_manager_get_current (CcDisplayConfigManager *self)
{
    ...
    return g_object_new (CC_TYPE_DISPLAY_CONFIG, "state", self->current_state, "connection", self->connection, NULL);
}
